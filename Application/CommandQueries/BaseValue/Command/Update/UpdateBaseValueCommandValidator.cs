...
{
    public class UpdateBaseValueCommandValidator : AbstractValidator<UpdateBaseValueCommand>
    {
        public UpdateBaseValueCommandValidator()
        {
            RuleFor(x => x.Title)
            ...
            RuleFor(x => x.UniqueName)
                .Must(x => x != null).WithMessage($"{ExceptionMessage.IsRequired}").Must(x => x is { Length: < 50 }).WithMessage($"{ExceptionMessage.MaxLenght}:50");
